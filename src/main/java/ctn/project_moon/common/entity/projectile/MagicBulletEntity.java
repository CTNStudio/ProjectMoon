    ...
    public void tick() {
        ...
        if(this.canTrack && !level().isClientSide){
            ...
            if (this.trackingTarget == null){
                //获取范围内实体
                AABB aabb = new AABB(this.getX()-this.TrackingDistance, this.getY()-1.0f, this.getZ()-this.TrackingDistance,
                        this.getX()+this.TrackingDistance, this.getY()+1.0f, this.getZ()+this.TrackingDistance);
                List<Entity> entityList = level().getEntitiesOfClass(Entity.class, aabb, (entity) -> {
...
//            }
    ...
    /**
     * 子弹实体不保存到世界文件，既退出重进后子弹会消失
     * TODO：如果保存到世界文件，退出重进会报错，可能需要重写save与load方法
     */
    @Override
